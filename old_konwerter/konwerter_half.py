#!/usr/bin/env python3
# konwerter_half.py
# Uzycie: python3 konwerter_half.py obrazek.png ekran1.h
#
# Wymaga: pip install Pillow
#
# Obrazek zostanie przeskalowany do 160x200 pikseli (pol mapy C64)
# Wynikowa tablica: bitmap 4000 bajtow + coloram 500 bajtow = 4500 bajtow
# 20 kolumn x 25 wierszy komorek 8x8
...
def convert(input_path, output_path):
    ...
    if img.size != (160, 200):
        print(f"Skaluje z {img.size} do 160x200...")
        img = img.resize((160, 200), Image.LANCZOS)

    pixels = list(img.getdata())

    # 160x200 px = 20 kolumn x 25 wierszy komorek 8x8
    COLS = 20
    ROWS = 25
    bitmap  = bytearray(COLS * ROWS * 8)  # 4000 bajtow
    coloram = bytearray(COLS * ROWS)      # 500 bajtow
    ...
    data = bitmap + coloram  # 4500 bajtow
    total = len(data)

    with open(output_path, "w") as f:
        f.write("#ifndef EKRAN1_H\n")
        f.write("#define EKRAN1_H\n\n")
        f.write(f"// Pol mapy C64: 160x200px, 20x25 komorek 8x8\n")
        f.write(f"// bitmap: 4000 bajtow [0..3999], coloram: 500 bajtow [4000..4499]\n")
        f.write(f"static const unsigned char ekran1[{total}] = {{\n")
    ...
    print(f"Zapisano: {output_path}  ({total} bajtow: {COLS*ROWS*8} bitmap + {COLS*ROWS} coloram)")

if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Uzycie: python3 konwerter_half.py obrazek.png ekran1.h")
        sys.exit(1)
