#!/usr/bin/env python3
# konwerter64px.py  — konwertuje obrazek 64x64px do kafla C64 hires
# Uzycie: python3 konwerter64px.py obrazek.png nazwa_kafla
# Wynik:  nazwa_kafla.h  z  static const unsigned char nazwa_kafla[576]
#
# Kafel: 64x64px = 8x8 komorek 8x8
# Format: [512B pikseli][64B kolorow] = 576B razem
# DRAW_TILE(nazwa, dest_cx, dest_cy, 8, 8);
#
# pip install Pillow
...
def convert(input_path, tile_name):
    ...
    with open(out_path, "w") as f:
        ...
        f.write(f"/* Kafel 64x64px (8x8 komorek), rozmiar=576B */\n")
        f.write(f"/* Uzycie: DRAW_TILE({tile_name}, dest_cx, dest_cy, 8, 8); */\n\n")
        f.write(f"static const unsigned char {tile_name}[576] = {{\n")
        f.write("    /* --- piksele (512B) --- */\n")
        for i in range(0, PIXEL_BYTES, 8):
            chunk = data[i:i+8]
            f.write("    " + ", ".join(f"0x{b:02X}" for b in chunk) + ",\n")
        f.write("    /* --- kolory (64B: gorny nibble=fg, dolny=bg) --- */\n")
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print(f"Uzycie: python3 konwerter64px.py obrazek.png nazwa_kafla")
        print(f"Przyklad: python3 konwerter64px.py grass.png grass")
        print(f"Wynik:    grass.h  z  DRAW_TILE(grass, x, y, 8, 8);")
        sys.exit(1)
