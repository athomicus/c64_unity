#!/usr/bin/env python3
# konwerter8px.py  — konwertuje obrazek 8x8px do kafla C64 hires
# Uzycie: python3 konwerter8px.py obrazek.png nazwa_kafla
# Wynik:  nazwa_kafla.h  z  static const unsigned char nazwa_kafla[9]
#
# Kafel: 8x8px = 1x1 komorek 8x8
# Format: [8B pikseli][1B kolorow] = 9B razem
# DRAW_TILE(nazwa, dest_cx, dest_cy, 1, 1);
#
# pip install Pillow
...
def convert(input_path, tile_name):
    ...
    with open(out_path, "w") as f:
        ...
        f.write(f"/* Kafel 8x8px (1x1 komorek), rozmiar=9B */\n")
        f.write(f"/* Uzycie: DRAW_TILE({tile_name}, dest_cx, dest_cy, 1, 1); */\n\n")
        f.write(f"static const unsigned char {tile_name}[9] = {{\n")
        f.write("    /* --- piksele (8B) --- */\n")
        for i in range(0, PIXEL_BYTES, 8):
            chunk = data[i:i+8]
            f.write("    " + ", ".join(f"0x{b:02X}" for b in chunk) + ",\n")
        f.write("    /* --- kolory (1B: gorny nibble=fg, dolny=bg) --- */\n")
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print(f"Uzycie: python3 konwerter8px.py obrazek.png nazwa_kafla")
        print(f"Przyklad: python3 konwerter8px.py grass.png grass")
        print(f"Wynik:    grass.h  z  DRAW_TILE(grass, x, y, 1, 1);")
        sys.exit(1)
